...
execution and the full decode Graph backend.
...
## Opt-in multistream execution

Add `--enable-nccl-ep-multistream` with `--enable-two-batch-overlap`
and/or `--enable-single-batch-overlap`. The option defaults to disabled and
requires single-node CUDA, DeepSeek V2/V3 block-128 FP8 experts, and the Triton
MoE runner. EPLB, PDMux, compile, memory saver and speculation are unsupported.
Each TBO lane owns its communication stream, group, handle and scratch.
Separate compute streams require independent Triton attention children,
attention TP=1 and dense TP=1. Other attention workspaces or unsupported
allocators retain the existing compute stream. Graph buckets and replays
remain serialized. Enabling overlap does not guarantee an end-to-end speedup;
the measured DeepSeek-V2-Lite workload currently regresses against serial.

See the [registered-test instructions](https://github.com/sgl-project/sglang/tree/main/test/nccl_ep_test)
for local lifecycle and multistream regressions. The native synthetic check is
in `test/manual/test_nccl_ep_synthetic.py`. Full-model benchmarks and Nsight
experiment drivers are maintained separately; they are not shipped here.
See [expert parallelism](/docs/advanced_features/expert_parallelism) for the
